...
kernel_config::()
{
    ...
    then
        if logicTrue $(profile_get_key kbuild-output-clean)
        then
            rm -rf $(profile_get_key kbuild-output)
            mkdir -p $(profile_get_key kbuild-output) 
        fi
        print_info 2 "KERNEL SOURCES: $(profile_get_key kernel-tree)"
        print_info 2 "KBUILD OUTPUT: $(profile_get_key kbuild-output)"
        ...
        if ! cat ${KBUILD_OUTPUT}/include/config/kernel.release 2>/dev/null \
            | egrep "^${KV_MAJOR}.${KV_MINOR}.${KV_PATCH}" &>/dev/null
        then
    ...
    # Other modules may require kernel options .. eg lvm2 support would
    # register options
