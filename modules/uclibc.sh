require @pkg_uClibc-${UCLIBC_VER}:null:uclibc_compile
...
uclibc::() {
	[ -e ${TEMP}/uclibc-output ] && rm -r ${TEMP}/uclibc-output
	mkdir -p ${TEMP}/uclibc-output
	cd ${TEMP}/uclibc-output
	genkernel_extract_package "uClibc-${UCLIBC_VER}"
